...
def report_fnr_fpr(y_true, y_pred, class_names):
    ...
    for i in range(len(cm)):
        ...
        fnr_list.append(fnr)
        fpr_list.append(fpr)
        support_list.append(FN + TP) 

    # Weighted 평균 계산
    total_support = sum(support_list)
    weighted_fnr = sum(fnr_list[i] * support_list[i] for i in range(len(fnr_list))) / total_support
    weighted_fpr = sum(fpr_list[i] * support_list[i] for i in range(len(fpr_list))) / total_support
    ...
    print(f"\nWeighted FNR: {weighted_fnr:.4f}")
    print(f"Weighted FPR: {weighted_fpr:.4f}")

    return df, weighted_fnr, weighted_fpr
...
def evaluate(model, loader, device, n_class, detailed=False, logf=None):
    ...
    elif detailed and n_class == 5:
        class_names = ["Normal", "Gambling", "Adult", "Investment", "Forum"]
        print("\n📊 Classification Report")

        # precision, recall, f1, support
        prec, recall, f1, support = precision_recall_fscore_support(all_labels, all_preds, labels=[0, 1, 2, 3, 4])
        total = np.sum(support)
